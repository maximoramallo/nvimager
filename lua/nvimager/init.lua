...
\usepackage{tikz-cd} % Drawings.
\usepackage{modiagram} % Chemistry diagrams.
...
local function drawImage(path, scaler, x, y, width, height)
    ...
    return vim.fn.jobstart(string.format( vim.g["plugindir"].."/imgt %d %d %d %d %s", x, y, width, height, path))
end
...
local function createTexPreview(equation, extId)
    local path = os.getenv("HOME").."/.cache/nvimager/"
    vim.fn.mkdir(path, "p")
